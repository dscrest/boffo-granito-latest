/* Container Plan snapshot (Plan Containerisation) — read-only card shared by
   ...
   stored on the quote; links to the editor at /quotes/:id/containerise. */
import { Link } from "react-router-dom";
import { parseContainerPlan, type Quote } from "@/data";
import { fmt } from "@/lib/format";
import { EmptyState } from "@/ui/States";

export function ContainerPlanCard({ quote }: { quote: Quote }) {
    ...
    return (
      <div className="card" style={{ padding: 14 }}>
        <EmptyState
          ...
          action={<Link className="btn" to={`/quotes/${quote.id}/containerise`}>Open planner</Link>}
        />
      </div>
    );
  }
  ...
  return (
    <div className="card">
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderBottom: "1px solid var(--border)" }}>
        ...
        <Link className="btn" style={{ marginLeft: "auto" }} to={`/quotes/${quote.id}/containerise`}>
          Open planner
        </Link>
      </div>
