...
- [x] **Group D** — Empty states + Retry + skeleton loaders (§3.3, §3.4) ✅ new `ui/States.tsx` (EmptyState/ErrorCard/SkeletonRows) applied to 10 list pages + App Suspense fallback
...
- [x] **Big #1b** — Mock→live remaining: Parties/CustomerDetail/ItemDetail (needs a Customer master API), Dashboard Activity + Ready-to-Load feeds, OrderForm/QuoteForm PARTIES+DESIGNS pickers (§0) ✅ new `customersApi.ts` (Customer+PaymentTerm FK, cached) + `useMasters()` hook; Parties saves real customers (PartyForm → CustomerInput), CustomerDetail/ItemDetail live + OperationLog entityId; Dashboard Activity ← OperationLog, Ready-to-Load + KPI derived from live orders; QuoteForm/OrderForm/QuoteDetail/QuotePrint pickers + address autofill from Customer master; live sidebar design/parties counts
